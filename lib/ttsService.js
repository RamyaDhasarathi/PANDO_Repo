...
async function speakWithEdgeTTS(text, generation, { onStart, onEnd, onError } = {}) {
  ...
  if (!blob) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 350); // 350ms max waiting time for network fetch
    ...
    try {
      ...
      clearTimeout(timeoutId);
      throw err; // Will trigger fallback to browser voice instantly
    }
  }
 ...
 * if network fetch takes >350ms or fails, instantly falls back to browser voice.
 */
